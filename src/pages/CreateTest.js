...
export default function CreateTest() {
    ...
    const Submit = async()=>{
        const hasEmptyObjectL1 = listenP1.filter(obj => Object.keys(obj).length === 0);
        const hasEmptyObjectL2 = listenP2.filter(obj => Object.keys(obj).length === 0);
        const hasEmptyObjectL3 = listenP3.filter(obj => Object.keys(obj).length === 0);
        const hasEmptyObjectL4 = listenP4.filter(obj => Object.keys(obj).length === 0);
        const hasEmptyObjectR1 = readP1.filter(obj => Object.keys(obj).length === 0);
        const hasEmptyObjectR2 = readP2.filter(obj => Object.keys(obj).length === 0);
        const hasEmptyObjectR3 = readP3.filter(obj => Object.keys(obj).length === 0);
        if(testname=="" || hasEmptyObjectL1 || hasEmptyObjectL2 || hasEmptyObjectL3 || hasEmptyObjectL4 || hasEmptyObjectR1 || hasEmptyObjectR2 || hasEmptyObjectR3){
            alert('Action cannot be done! You have not input enough questions and datas of a test.')
            return;
        }
        const data = {
            Part1:listenP1,
  ...
  return (
    <div>
        <div style={{display:'flex', marginLeft:'10%',}}>
            ...
            <button onClick={() => Submit()} type="button" class="btn btn-light" 
            style={{backgroundColor: '#F88C19', color: '#fff', width: 80, marginLeft: 30}}>
                Create
            </button>  
        </div>
        ...
        <div style={{display: 'flex'}}>
            <div style={{width: '70%'}}>
                ...
                <div style={{marginLeft: '10%', width: '80%'}} className='testname-wrapper'>
                    <label style={{fontSize: 20, color: 'black', fontWeight: 'bold'}}>
                        Test name:
                        <input className="customInput" onChange={(e) => setTestName(e.target.value)}/>
                    </label>
                </div>
                {(sign=='L1')&&
                ...
                listenP2.map((each,key)=>{
                    return(
                        <ListenPart2 flag={'Test'} index={key+6}
                        complete = {(data)=>{
            ...
            <div>
        {sign==='L1'&&
        <NoteCard title={`${sign}: Photographs`} 
        ...
        {sign==='L2'&&
        <NoteCard title={`${sign}: Question & Response`} 
        ...
        {sign==='L3'&&
        <NoteCard title={`${sign}: Short Conversations`} 
        ...
        {sign==='L4'&&
        <NoteCard title={`${sign}: Short Talks`} 
        ...
        {sign==='R1'&&
        <NoteCard title={`${sign}: Incomplete Sentences`} 
        ...
        {sign==='R2'&&
        <NoteCard title={`${sign}: Text Completion`} 
        ...
        {sign==='R3'&&
        <NoteCard title={`${sign}: Reading Comprehension`} 
