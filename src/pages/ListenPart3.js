...
function ListenPart3({flag, index, complete, item}) {
  ...
  const validateData = () => {
    ...
    if(errorFields.length > 0|| !isAudioValid || number.length != 3|| hasEmptyQ || hasEmptyScript || hasNoTrueAnswer){
  ...
  return (
    <div className='addQuestion'>
      {(flag=='submit')?<h2>Add Question Listening Part 3</h2>:<h2>Question {index+1} </h2>}
     {(flag==='see')&&<>
