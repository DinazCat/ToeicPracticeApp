...
function SpeakPart5({item,complete,flag,index}) {
  ...
  return (
    <div className='addQuestion'>
      {(flag=='submit')?<h2>Add Question Speaking Part 5</h2>:<h2>Question {index+1} </h2>}
     ...
     <div className='flex-column'>
      <div>
        ...
        <label style={{display: 'flex'}}>
          <textarea className="customInput" value={item.Question} onChange={(e) => setQuestion(e.target.value)} rows="3" />
        </label>
      </div>
      <div>
        Sample Answer:
        <label style={{display: 'flex'}}>
          <textarea className="customInput" value={item.Explain.SampleAnswer} onChange={(e) => setSampleAnswer(e.target.value)} rows="8" />
        </label>
      </div>
      <div>
        Tips:  
     ...
     <div className='flex-column'>
      <div>
        Question:
        <label style={{display: 'flex'}}>
          <textarea className="customInput" value={question} onChange={(e) => setQuestion(e.target.value)} rows="3" />
        </label>
      </div>
      <div>
        Sample Answer:
        <label style={{display: 'flex'}}>
          <textarea className="customInput" value={sampleAnswer} onChange={(e) => setSampleAnswer(e.target.value)} rows="8" />
        </label>
      </div>
      <div>
        Tips:  
