...
function TestView() {
  ...
  return (
   ...
   <div style={{display:'flex', marginLeft:'10%'}}>
        ...
        {flag=='fix' && 
        <button onClick={() => onSave()} type="button" class="btn btn-light" 
        style={{backgroundColor: '#F88C19', color: '#fff', marginLeft: 20, width: 80}} >
            Save
        </button>}
    </div>
    ...
    <div style={{display: 'flex'}}>
        ...
        <div style={{marginLeft: '10%', width: '80%'}} className='testname-wrapper'>
            <label style={{fontSize: 20, color: 'black', fontWeight: 'bold'}}>
                Test name:
                <textarea className="customInput"value={testname} onChange={(e) => setTestName(e.target.value)}/>
            </label>
        </div>
