...
function ListenPart4({flag, index, complete, item}) {
  ...
  const validateData = () => {
    ...
    if(number.length != 3) quantityError= "The number of questions in this part must be 3!"
    if(errorFields.length > 0|| number.length != 3 || !isAudioValid || hasEmptyQ || hasEmptyScript || hasNoTrueAnswer){
  ...
  return (
    <div className='addQuestion'>
      {(flag=='submit')?<h2>Add Question Listening Part 4</h2>:<h2>Question {index+1} </h2>}
    {(flag==='see')&&<>
