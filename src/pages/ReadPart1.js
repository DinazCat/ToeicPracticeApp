...
function ReadPart1({flag, index, complete, item}) {
  ...
  const validateData = () => {
    ...
    if(selectedAnswer==null || selectedAnswer==-1 || textR1=="" || textR2=="" || textR3=="" || textR4==""){
  ...
  return (
    <div className='addQuestion'>
      {(flag=='submit')?<h2>Add Question Reading Part 1</h2>:<h2>Question {index+1} </h2>}
      {(flag==='see')&&<>
      <div>
        Question:
        <textarea className="customInput" value={item.Question} onChange={(e) => setQuestion(e.target.value)} rows="2" />
      </div>
      <div style={{marginTop:10, marginBottom:10,display:'grid'}}>
      <label>Answer:</label>
      <div style={{display:'inline-flex', marginLeft:5, marginBottom: 10}}>
        <button className="btn rounded-circle" style={{backgroundColor: item.Answer[0].status && '#5DA110', marginRight: 10, border: '1px solid black', height: 40}}
        onClick={() => {handleAnswerChange(0)}}>A</button>
          <input className="customInput" type='text' onChange={(e) => setTextR1(e.target.value)} value={item.Answer[0].script} id='TR'></input> 
        </div>
        <div style={{display:'inline-flex', marginLeft:5, marginBottom: 10}}>
        <button className="btn rounded-circle" style={{backgroundColor: item.Answer[1].status && '#5DA110', marginRight: 10, border: '1px solid black', height: 40}}
         onClick={() => {handleAnswerChange(1)}}>B</button>
        <input className="customInput" type='text' onChange={(e) => setTextR2(e.target.value)} value={item.Answer[1].script} id='TR'></input>
        </div>
        <div style={{display:'inline-flex', marginLeft:5, marginBottom: 10}}>
        <button className="btn rounded-circle" style={{backgroundColor: item.Answer[2].status && '#5DA110', marginRight: 10, border: '1px solid black', height: 40}}
        onClick={() => {handleAnswerChange(2)}}>C</button>
        <input className="customInput" type='text' onChange={(e) => setTextR3(e.target.value)} value={item.Answer[2].script} id='TR'></input> 
        </div>
        {item.Answer[3]&&
        <div style={{display:'inline-flex', marginLeft:5, marginBottom: 10}}>
        <button className="btn rounded-circle" style={{backgroundColor: item.Answer[3].status && '#5DA110', marginRight: 10, border: '1px solid black', height: 40}}
        onClick={() => {handleAnswerChange(2)}}>D</button>
        <input className="customInput" type='text' onChange={(e) => setTextR4(e.target.value)} value={item.Answer[3].script} id='TR'></input> 
        </div>
        }
      </div>
      ...
      <div>
        Question:
        <textarea className="customInput" value={question} onChange={(e) => setQuestion(e.target.value)} rows="2" />
      </div>
